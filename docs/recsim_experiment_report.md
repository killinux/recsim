...
### 6.1 LTS（长期满意度）— reward 波动剧烈是核心信号

```
Step  6: reward=   88.27    ← 高 clickbait 文档，短期爆发
Step  9: reward=    1.09    ← 满意度下降后，连好内容也没人看了
```

reward 就是 **engagement（参与度）**，比如视频观看时长。它由两个因素共同决定：

```
engagement = 文档吸引力 × 用户满意度
```

- **短期看**：推荐 clickbait（高 `clickbait_score`）能拿到 88 这样的高 reward
- **长期看**：clickbait 会**偷偷降低**用户的隐藏满意度（Agent 看不到），导致后续所有内容的 engagement 都缩水

**怎么判断策略好坏**：不是看单步 reward 高不高，而是看 **20 步总 reward**。一个好的 RL 策略会适当推荐"甘蓝"（优质内容），牺牲短期 reward 换取满意度回升，让后续每一步的 reward 都更高。

随机策略拿了 **244.64**，这是基线。论文中 RL 策略能拿到更高的总 reward。

### 6.2 Interest Evolution（兴趣演化）— 看点击率和会话时长

```
reward=4.00  →  用户点了，看了 4 分钟完整视频
reward=0.00  →  用户没点任何东西（slate 上的文档都不感兴趣）
```

**关键指标**：

- **点击率** = 9/20 = 45%，说明随机推荐有一半以上不命中
- **总 reward 32.00** = 只看了 32 分钟视频（预算 200 分钟）

这个环境的特点是用户兴趣会**随推荐内容漂移**。如果连续推荐同一类内容，用户兴趣会向该方向偏移。好的策略应该：

1. 利用已知的用户兴趣向量（这个环境里 Agent **能看到**兴趣），推荐高匹配文档
2. 推荐高质量文档延长会话（`budget += quality × watch_time`）

随机策略两个都做不到，所以 20 步才用了 32 分钟预算就近似原地踏步。

### 6.3 Interest Exploration（兴趣探索）— 看能否发现用户真正偏好
...
**点击率** = 6/20 = 30%，是三个环境里最差的。

因为这个环境里用户兴趣是**隐变量**（Agent 看不到），而且**不会变化**。它本质是一个**多臂赌博机问题**：

- 有 M 个主题，每个主题对当前用户的真实吸引力不同
- Agent 不知道用户喜欢哪个主题，只能通过"推荐 → 看是否点击"来试探
- 随机策略 = 瞎猜，30% 的点击率就是瞎猜的水平
...
| 环境 | 看什么 | 随机策略问题 | 好策略应该做什么 |
|:-----|:-------|:------------|:---------------|
| **LTS** | 总 engagement | 不知道在消耗满意度 | 短期忍住不推 clickbait |
| **IE** | 点击率 + 会话时长 | 不利用已知兴趣 | 推匹配+高质量内容延长会话 |
| **IEx** | 点击率 | 瞎猜主题 | 先探索再利用 |

三个环境分别考察了推荐系统 RL 的三个核心挑战：**长期规划**、**组合动作空间**、**探索-利用**。随机策略在所有场景下都是最差基线，这正是论文想说明的——推荐系统需要 RL 来做序列决策。
